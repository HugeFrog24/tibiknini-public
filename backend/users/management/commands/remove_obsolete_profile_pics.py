    ...
    def handle(self, *args, **kwargs):
        ...
        if profile_images_directory.exists() and profile_images_directory.is_dir():
            current_profile_images = (
                Profile.objects.exclude(image__isnull=True)
                .exclude(image__exact="")
                .values_list("image", flat=True)
            )
            current_profile_images = {
                (media_root / img).resolve() for img in current_profile_images
            }

            for image_path in profile_images_directory.glob("**/*"):
                if (
                    image_path.is_file()
                    and image_path.resolve() not in current_profile_images
                ):
                    image_path.unlink()

        self.stdout.write(
            self.style.SUCCESS(
                "Obsolete profile pictures have been successfully removed."
            )
        )
