...
class ProfileAPITests(APITestCase):
    ...
    def test_profile_image_update_view(self):
        ...
        response = self.client.patch(
            reverse("profile-image-update", kwargs={"username": self.user1.username}),
            {"image": image},
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_profile_image_delete_view(self):
        self.client.login(username="user1", password="testpassword")
        self.user1.profile.image = SimpleUploadedFile(
            name="test_image.jpg", content=b"", content_type="image/jpeg"
        )
    ...
    def test_follow_toggle_view(self):
        ...
        response = self.client.post(
            reverse("follow_toggle"), {"username": self.user2.username}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertTrue(
            Follow.objects.filter(
                follower=self.user1.profile, following=self.user2.profile
            ).exists()
        )
        response = self.client.post(
            reverse("follow_toggle"), {"username": self.user2.username}
