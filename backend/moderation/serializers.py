...
class ContentReportSerializer(serializers.ModelSerializer):
    ...
    reason = serializers.PrimaryKeyRelatedField(queryset=ReportReason.objects.filter(is_active=True))
    reported_content_str = serializers.SerializerMethodField()
    content_type_str = serializers.SerializerMethodField()
    
    class Meta:
        model = ContentReport
        fields = ['id', 'content_type', 'content_type_str', 'object_id', 'reason', 
                 'description', 'reported_at', 'reported_content_str']
    ...
    def validate(self, data):
        ...
        try:
            content_obj = data['content_type'].get_object_for_this_type(id=data['object_id'])
        except:
            raise serializers.ValidationError("The reported content does not exist")
        
        # Prevent self-reporting
        if (data['content_type'].model_class().__name__ == 'CustomUser' and 
            data['object_id'] == self.context['request'].user.id):
