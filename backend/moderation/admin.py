...
from .models import ReportReason, ContentReport, ModerationAction, UserWarning

@admin.register(ReportReason)
class ReportReasonAdmin(admin.ModelAdmin):
    list_display = ('name', 'is_active', 'created_by', 'created_at', 'last_modified_by', 'last_modified_at')
    list_filter = ('is_active', 'created_at', 'last_modified_at')
    search_fields = ('name', 'description')
    readonly_fields = ('created_by', 'created_at', 'last_modified_by', 'last_modified_at')
...
class ModerationActionInline(admin.TabularInline):
    ...
    extra = 0
    readonly_fields = ('action_type', 'performed_by', 'performed_at', 'view_state_changes')
    fields = ('action_type', 'performed_by', 'performed_at', 'view_state_changes', 'notes')
    can_delete = False
    max_num = 0
...
class ContentReportAdmin(admin.ModelAdmin):
    list_display = ('id', 'reporter', 'content_link', 'reason', 'reported_at', 'verdict', 'reviewed_by', 'action_taken')
    ...
    def hide_content(self, request, queryset):
        for report in queryset:
            try:
                ...
                messages.success(request, f'Successfully hidden content for report #{report.id}')
            except Exception as e:
                messages.error(request, f'Failed to hide content for report #{report.id}: {str(e)}')
    ...
    def warn_user(self, request, queryset):
        for report in queryset:
            try:
                ...
                messages.success(request, f'Successfully issued warning for report #{report.id}')
            except Exception as e:
                messages.error(request, f'Failed to issue warning for report #{report.id}: {str(e)}')
    ...
    def ban_user(self, request, queryset):
        for report in queryset:
            try:
                ...
                messages.success(request, f'Successfully banned user for report #{report.id}')
            except Exception as e:
                messages.error(request, f'Failed to ban user for report #{report.id}: {str(e)}')
...
class ModerationActionAdmin(admin.ModelAdmin):
    list_display = ('performed_at', 'action_type', 'performed_by', 'get_related_item', 'notes')
    list_filter = ('action_type', 'performed_at', 'performed_by')
    search_fields = ('notes', 'performed_by__username')
    readonly_fields = ('performed_at', 'performed_by', 'action_type', 'report', 
                      'report_reason', 'previous_state', 'new_state')
